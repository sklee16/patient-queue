...
public class Part2b {
	...
	private static void check_emergency() {
		...
		if (random.nextDouble() < Parameters.p_emergency) {
			...
			// -----
			int selected_queue = random.nextInt(Q.length);
			int selected_patient = random.nextInt(Q[selected_queue].size);
			Patient emerg_Patient = Q[selected_queue].dequeue(selected_patient);

			//get
			Q[3].push(emerg_Patient);
		}
	}
	...
	private static void check_promotion() {
		...
		for (int i = 0; i < Q[1].size; i++) {
			if (Q[1].get(i).wait_time(time) >= 2 * Parameters.l12_escalation_delay) {
				Patient promoted = Q[1].dequeue(i);
				Q[2].push(promoted);//enqueue
				return;
			}
		}
	}
	...
	private static void check_walkout() {
		...
		if (random.nextDouble() < Parameters.p_walkout) {
			// TODO ATTENTION: CODE NEEDED HERE
			// select a random long-waiting patient in level 1
			// they leave the clinic
			// -----
			while(true) {
				int ranPatient = random.nextInt(Q[0].size);

				if (Q[0].get(ranPatient).wait_time(time) > Parameters.l1_walkout_delay) {
					Q[0].dequeue(ranPatient);
					break;
				}
			}
		}
